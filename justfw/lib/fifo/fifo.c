...
fifo_s_t *fifo_s_create(int uint_cnt)
{
  ...
  p_fifo = (fifo_s_t *)JUST_MALLOC(sizeof(fifo_s_t));
  if (NULL == p_fifo)
  {
  ...
  p_base_addr = JUST_MALLOC(uint_cnt);
  if (NULL == p_base_addr)
  {
    //! Allocate Failure, exit now
    free(p_fifo);
...
fifo_t *fifo_create(char unit_size, int unit_cnt)
{
  ...
  //! Allocate Memory for pointer of new FIFO Control Block.
  p_fifo = (fifo_t *)JUST_MALLOC(sizeof(fifo_t));
  if (NULL == p_fifo)
  {
    //! Allocate Failure, exit now.
    return (NULL);
  ...
  p_base_addr = JUST_MALLOC(unit_size * unit_cnt);
  if (NULL == p_base_addr)
  {
    //! Allocate Failure, exit now.
    free(p_fifo);
